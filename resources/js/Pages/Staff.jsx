        ...
        <Layout settings={settings} locale={locale}>
            ...
            <section className="py-[clamp(54px,7vw,88px)] bg-brand-muted bg-dot-pattern">
                <div className="max-w-[1180px] mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[18px]">
                    {staff.map((member, i) => (
                        <article
                            className="bg-white border border-border-light rounded-lg shadow-card overflow-hidden transition-[box-shadow,transform] duration-[220ms] hover:-translate-y-1 hover:shadow-card-lg reveal"
                            key={member.name}
                        ...
                        >
                            ...
                            <div className="p-5">
                                <h2 className="mb-1 text-navy-800 font-extrabold text-[1.05rem] leading-[1.25]">{member.name}</h2>
                                ...
                                {member.experience && <span className="block text-slate-400 text-[0.82rem]">{member.experience}</span>}
                            </div>
                        </article>
                    ))}
                </div>
